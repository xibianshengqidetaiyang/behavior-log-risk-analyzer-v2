...
**上网行为日志威胁初筛工具｜个人项目 / GitHub**

- 基于 Python 设计并实现上网行为日志威胁初筛工具，支持 IOC 多字段匹配、异常行为规则检测与风险分级。
- 支持自动识别不同日志表头，并提供手动字段映射兜底，增强不同日志格式下的适配能力。
- 可输出 CSV/Markdown 结构化研判结果，用于辅助人工复核、周报整理与日志分析提效。
